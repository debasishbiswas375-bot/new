import streamlit as st
import hashlib
import psycopg2

def app():
    st.title("🔐 Access Portal")
    tab1, tab2 = st.tabs(["Sign In", "Full Business Registration"])

    with tab2:
        st.subheader("Register Business")
        with st.form("reg_form"):
            u = st.text_input("Username*")
            e = st.text_input("Email")
            p = st.text_input("Password*", type="password")
            
            if st.form_submit_button("Register & Sync"):
                try:
                    conn = psycopg2.connect(st.secrets["DATABASE_URL"])
                    cur = conn.cursor()
                    # Syncs with auth_user and the newly created user_id_id column
                    cur.execute("INSERT INTO auth_user (username, password, email, is_active, date_joined) VALUES (%s, %s, %s, True, NOW()) RETURNING id", (u, p, e))
                    uid = cur.fetchone()[0]
                    cur.execute("INSERT INTO profiles (user_id_id, username, email, points) VALUES (%s, %s, %s, 100)", (uid, u, e))
                    conn.commit()
                    st.success(f"Success! {u} registered.")
                    conn.close()
                except Exception as ex:
                    st.error(f"Sync failed: {ex}")
